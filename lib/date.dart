...
class Date {
  ...
  static const List<int> _dpm = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
  static const List<int> _dpmLpY = [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
  ...
  int compareTo(Date date){
    if(date.year() == _year && date.month() == _month && date.day() == _day) return 0;
    if(date.year() < _year) return 1;
    if(date.year() == _year && date.month() < _month) return 1;
    if(date.year() == _year && date.month() == _month && date.day() < _day) return 1;
    return -1;
  }
  ...
  _assert(){
    ...
    if (_month < 1 || _month > 12) {
      throw Exception("Out-of-bounds: [$_month] Month must be in [1, 12] range!");
    }

    if(_month != 2){
      if(_isLeap()){
        if(_day < 1 || _day > _dpmLpY[_month-1]){
          throw Exception("Out-of-bounds: [$_day] Days must be in [1,${_dpmLpY[_month-1]}] for a leap year!");
        }
      }
      else{
        if(_day < 1 || _day > _dpm[_month-1]){
          throw Exception("Out-of-bounds: [$_day] Days must be greater in [1,${_dpmLpY[_month-1]}] for a regular year!");
        }
      }
    }
    else {
