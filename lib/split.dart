...
class _CrossSplit extends State<CrossSplit>{
  ...
  void _vSync(PanNotification notification){
    vRatio.ratio += notification.details.delta.dx / (notification.constraints.maxWidth - vRatio.grabSize);
    if(vRatio.ratio < 0) {
      vRatio.ratio = 0.0;
  ...
  void _hSync(PanNotification notification){
    hRatio.ratio += notification.details.delta.dy / (notification.constraints.maxHeight - hRatio.grabSize);
    if(hRatio.ratio < 0) {
      hRatio.ratio = 0.0;
  ...
  bool onSync(SyncNotification notification){
    setState(() {
      if(notification is PanNotification){
        ...
        double x1 = vRatio.ratio * (notification.constraints.maxWidth - vRatio.grabSize);
        double x2 = vRatio.ratio * (notification.constraints.maxWidth - vRatio.grabSize) + vRatio.grabSize;
  ...
  Widget build(BuildContext context){
    return NotificationListener<SyncNotification>(
      ...
      child: Column(
        children: [
          Split(
            "hAll", hRatio, SplitDirection.horizontal, 
            color: Colors.green, 
            topOrLeft: Row( 
              children: [
                Split("vTop", vRatio, SplitDirection.vertical, 
                color: Colors.red,
...
class Split extends StatelessWidget {
  ...
  Split(this._name, this._ratio, this._direction, {this.color=Colors.grey, this.topOrLeft=const Placeholder(), this.bottomOrRight=const Placeholder()});

  Map getSizes(BoxConstraints constraints){
    return {
      "sb1_w": _direction == SplitDirection.vertical ? (constraints.maxWidth - _ratio.grabSize)*_ratio.ratio : null, 
      "sb1_h": _direction == SplitDirection.horizontal ? (constraints.maxHeight - _ratio.grabSize)*_ratio.ratio : null,
      "sb2_w": _direction == SplitDirection.vertical ? (constraints.maxWidth - _ratio.grabSize)*(1-_ratio.ratio) : null, 
      "sb2_h": _direction == SplitDirection.horizontal ? (constraints.maxHeight - _ratio.grabSize)*(1-_ratio.ratio) : null,
    };
  }
      ...
      child: LayoutBuilder(
        builder: (BuildContext context, BoxConstraints constraints) {
          ...
          return SizedBox(
            ...
            child: Flex(
              direction: _direction == SplitDirection.horizontal ? Axis.vertical : Axis.horizontal,
              children: [
                SizedBox(
                  width: sizes["sb1_w"],
                  height: sizes["sb1_h"],
                ...
                GestureDetector(
                  ...
                  child: Container(
                    height: _direction == SplitDirection.horizontal ? _ratio.grabSize : constraints.maxHeight,
                    width: _direction == SplitDirection.horizontal ? constraints.maxWidth : _ratio.grabSize,
                    color: color,
                  ),
                  onPanUpdate: (DragUpdateDetails details) {
                    PanNotification(_name, details, constraints).dispatch(context);
                  },
                  onTapDown: (TapDownDetails details) {
                    StartNotification(_name, details, constraints).dispatch(context);
                  }
                ),
                SizedBox(
                  width: sizes["sb2_w"],
